...
import { i as category, e as eq, n as novel, b as and, c as desc } from "./schema_98e5FuKX.mjs";
...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const pageSize = 24;
  const currentCategorySlug = null;
  const categories = await db.select().from(category).orderBy(category.name);
  let conditions = [eq(novel.published, true)];
  if (statusFilter === "ongoing") {
    conditions.push(eq(novel.status, "ongoing"));
  } else if (statusFilter === "completed") {
    conditions.push(eq(novel.status, "completed"));
  }
  const [{ total }] = await db.select({ total: count() }).from(novel).where(and(...conditions));
  const totalPages = Math.ceil(total / pageSize);
  const novels = await db.select().from(novel).where(and(...conditions)).orderBy(desc(novel.viewCount)).limit(pageSize).offset((page2 - 1) * pageSize);
  const r2Domain = env.R2_DOMAIN;
  return renderTemplate`${renderComponent($$result, "Layout", $$Layout, { "title": "All Novel Genres | Wuxia Dreams", "description": "Explore all wuxia, xianxia, and fantasy novels across different categories." }, { "default": async ($$result2) => renderTemplate` ${maybeRenderHead()}<main class="mx-auto max-w-6xl px-4 py-12"> <div class="flex flex-col md:flex-row gap-8 items-start">  <aside class="w-full md:w-56 lg:w-64 shrink-0"> <div class="sticky top-24 flex flex-col gap-1"> <h2 class="mb-4 text-xl font-bold tracking-tight">Categories</h2> <div class="flex flex-row md:flex-col gap-2 overflow-x-auto md:overflow-visible pb-4 md:pb-0 -mx-4 px-4 md:mx-0 md:px-0 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]"> <a${addAttribute(`/genres?status=${statusFilter}`, "href")}${addAttribute(`flex-shrink-0 px-4 py-2.5 rounded-xl text-sm font-medium transition-colors ${"bg-primary text-primary-foreground shadow-sm"}`, "class")}>
All Genres
</a> ${categories.map((c) => renderTemplate`<a${addAttribute(`/genre/${c.slug}?status=${statusFilter}`, "href")}${addAttribute(`shrink-0 px-4 py-2.5 rounded-xl text-sm font-medium transition-colors ${currentCategorySlug === c.slug ? "bg-primary text-primary-foreground shadow-sm" : "bg-card hover:bg-muted text-muted-foreground hover:text-foreground"}`, "class")}> ${c.name} </a>`)} </div> </div> </aside>  <div class="flex-1 min-w-0"> <div class="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4"> <h1 class="text-3xl font-bold tracking-tight">
All Novels
<span class="ml-3 text-lg font-normal text-muted-foreground">(${total})</span> </h1>  <div class="flex items-center gap-2 rounded-full border border-border/60 bg-card p-1 shadow-sm shrink-0"> <a${addAttribute(`?status=all`, "href")}${addAttribute(`px-4 py-1.5 rounded-full text-sm font-medium transition-colors ${statusFilter === "all" ? "bg-primary text-primary-foreground shadow-sm" : "text-muted-foreground hover:text-foreground hover:bg-muted/50"}`, "class")}>
All
...
}, "/Users/yangfuzhang/Desktop/wuxiadreams/wuxiadreams.com/src/pages/genres/index.astro", void 0);
const $$file = "/Users/yangfuzhang/Desktop/wuxiadreams/wuxiadreams.com/src/pages/genres/index.astro";
const $$url = "/genres";
