...
import { o as or, l as like, f as author, h as asc, c as desc, e as eq } from "./schema_98e5FuKX.mjs";
...
async function GET({
  ...
  if (sortBy === "name") {
    ...
    orderByColumn = sortOrder === "asc" ? asc(author.createdAt) : desc(author.createdAt);
  }
  try {
    const [data, totalResult] = await Promise.all([
      db.select().from(author).where(queryFilter).orderBy(orderByColumn).limit(pageSize).offset(offset),
